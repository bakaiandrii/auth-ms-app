...
module.exports = {
  login: async (req, res) => {
    try {
      ...
      if (role === appConfig.USER_ROLE_TYPE1) {
        await generateTokenWithRole(res, role, _id);
      } else if (role === appConfig.USER_ROLE_TYPE2) {
        await generateTokenWithRole(res, role, _id);
      } else if (role === appConfig.USER_ROLE_TYPE3) {
        await generateTokenWithRole(res, role, _id);
      }
    } catch (err) {
  ...
  refresh: async (req, res) => {
    try {
      ...
      Jwt.verify(refresh_token, appConfig.JWT_REFRESH_SECRET, err => {
  ...
  forgot: async (req, res, next) => {
    try {
      ...
      await emailService.sendMail(email, appConfig.FORGOT_PASS_URL + token);
